...
fun FirebaseAuthException.toAuthException(): AuthException {
    return when (this.errorCode) {
        "ERROR_INVALID_EMAIL" ->
            com.lumen1024.domain.AuthException.WrongFormatEmail(this.message)

        "ERROR_WEAK_PASSWORD" ->
            com.lumen1024.domain.AuthException.ShortPassword(this.message)

        "ERROR_EMAIL_ALREADY_IN_USE" -> com.lumen1024.domain.AuthException.EmailAlreadyExist(this.message)

        "ERROR_INVALID_CREDENTIAL" ->
            com.lumen1024.domain.AuthException.IncorrectCredentials(this.message)

        else -> com.lumen1024.domain.AuthException.Unknown(this.message)
    }
}
